...
class TwoLayerNN:
    ...
    def _initialize_parameters(self,n_features):
        self.W1 = random((self.hidden_units,n_features), 0.01)
        self.b1 = zeros((self.hidden_units, 1))
        self.W2 = random((1,self.hidden_units), 0.01)
        self.b2 = zeros((1,1))
    ...
    def _backward(self, Y, A2,caches):
        ...
        dA1, dW2, db2 = linear_activation_backward(dA2, caches[1], sigmoid_backward)
        _, dW1, db1 = linear_activation_backward(dA1, caches[0], relu_backward)
    ...
    def fit(self,X_test: np.ndarray, Y_test: np.ndarray):
        n_features,m = X_test.shape
        
        self._initialize_parameters(n_features)

        for epoch in range(self.epochs):
            # forward propogation
            prediction, caches = self._forward(X_test)
            # compute loss
            cost = self._compute_loss(Y_test, prediction)
            # backward propagation
            self._backward(Y_test, prediction, caches)
            # Gradient descent
